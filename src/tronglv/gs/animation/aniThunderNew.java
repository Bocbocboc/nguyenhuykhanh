...
public class aniThunderNew extends AnimateMySprite{
	...
	public aniThunderNew(float pX, float pY, float pWidth, float pHeight,
			...
			if(this.timeCount <= Share.row){
				if(this.setDoc){
					...
					if(this.timeCount < 5){
						...
						this.y += h;
						aniThunderNew.this.setHeight(this.h*(this.timeCount+1));
					...
					if(this.timeCount < 5){
						...
						this.x += h;
						aniThunderNew.this.setWidth(this.h*(this.timeCount+1));
	...
	public void removeThunder(){
		ResourcesManager.getInstance().activity.runOnUpdateThread(new Runnable() {			
			...
			public void run() {
				...
				aniThunderNew.entity.detachChild(aniThunderNew.this);
			}
		});
	}
}
